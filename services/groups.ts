...
export async function updateGroup(groupId: string, data: { name?: string; description?: string | null; sport_type?: string | null; type?: 'casual' | 'competition_org' | 'team' | 'venue_operator' }) {
  const { data: group, error } = await supabase
    .from('groups')
